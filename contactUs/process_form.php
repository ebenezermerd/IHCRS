...
if ($result->num_rows > 0) {
    ...
    echo '<input type="text" id="address-line1" name="address_line1" value="' . $row['address_line1'] . '" readonly>';
    echo '<label for="postcode">Postcode:</label>';
    echo '<input type="text" id="postcode" name="postcode" value="' . $row['postcode'] . '" readonly>';
    ...
    echo '<option value="yes"' . ($row['transport_requirement'] == 'yes' ? ' selected' : '') . '>Yes</option>';
    echo '<option value="no"' . ($row['transport_requirement'] == 'no' ? ' selected' : '') . '>No</option>';
    echo '</select>';
